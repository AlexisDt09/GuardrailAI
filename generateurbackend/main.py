...
# ===============================================
# 2. SECTION DES MODÈLES DE DONNÉES (PYDANTIC)
# ===============================================
# Toutes les définitions de classes viennent ici, AVANT d'être utilisées.
...
# ===============================================
# 3. INITIALISATION DE L'APPLICATION ET MIDDLEWARE
# ===============================================
app = FastAPI(title="API Garde-Corps v11.0 (Unified)", version="11.0.0")

is_gemini_configured = False # Drapeau pour la configuration de l'API
...
def parse_platine_data(platine_string: str) -> Optional[PlatineDetails]:
    ...
    try:
        ...
        return PlatineDetails(
            longueur=dims[0],
            largeur=dims[1],
            epaisseur=dims[2],
            nombre_trous=int(trous[0]),
            diametre_trous=trous[1],
            entraxe_longueur=entraxes[0],
            entraxe_largeur=entraxes[1]
        )
    ...
    try:
        for i, morceau_data in enumerate(data.morceaux):
            ...
            for sec_idx in section_indices:
                ...
                jonction_gauche_type = structure_items[sec_idx - 1].type if sec_idx > 0 else "rien"
                jonction_droite_type = structure_items[sec_idx + 1].type if sec_idx < len(structure_items) - 1 else "rien"

                is_extremite_gauche = (sec_idx == 1 and structure_items[0].type != 'section')
                is_extremite_droite = (sec_idx == len(structure_items) - 2 and structure_items[-1].type != 'section')

                deduction_gauche = dims_map.get(jonction_gauche_type, 0) / (1 if is_extremite_gauche else 2)
                deduction_droite = dims_map.get(jonction_droite_type, 0) / (1 if is_extremite_droite else 2)
        ...
        for m in data.morceaux:
            for s_item in m.structure:
                if s_item.type == 'poteau':
                    total_poteaux += 1
                elif s_item.type == 'liaison':
                    total_liaisons += 1

        if total_poteaux > 0:
            nomenclature.append(NomenclatureItem(item="Poteaux", details=data.poteau_dims, quantite=total_poteaux, longueur_unitaire_mm=data.hauteur_totale))
        if total_liaisons > 0:
            nomenclature.append(NomenclatureItem(item="Liaisons", details=data.liaison_dims, quantite=total_liaisons, longueur_unitaire_mm=data.hauteur_totale))
        ...
        final_data = FinalPlanData(
            description_projet=f"Garde-corps détaillé en {data.nombre_morceaux} morceau(x).",
            nomenclature=nomenclature,
            morceaux=final_morceaux,
            hauteur_totale=data.hauteur_totale,
            hauteur_lisse_basse=data.hauteur_lisse_basse,
            poteau_dims=data.poteau_dims,
            liaison_dims=data.liaison_dims,
            lissehaute_dims=data.lissehaute_dims,
            lissebasse_dims=data.lissebasse_dims,
            barreau_dims=data.barreau_dims,
            platine_details=platine_details,
            remplissage_type=data.remplissage_type,
            remplissage_details=remplissage_details
        )
